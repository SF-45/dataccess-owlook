...
public class TableViewForTableData extends TableView<DataEntity> {
    ...
    private FindActionDelay() {
      action = () -> {
        ...
        if (currentSearchText.get() == "" || currentSearchText.get() == null) {
          ...
          getItems().clear();
          getItems().addAll(
              currentItems.filtered(dataEntity -> dataEntity.findLike(currentSearchText.get())));
        }
  ...
  private void createColumn(FieldView field) {
    ...
    column.setCellValueFactory(dataEntity -> new SimpleStringProperty(
        dataEntity.getValue().getValue(field.getFieldName())));
